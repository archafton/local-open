...
To add the new committee tables to the database, create a migration script:
...
CREATE INDEX IF NOT EXISTS idx_committees_parent_id ON committees(parent_committee_id);
CREATE INDEX IF NOT EXISTS idx_committees_normalized_name ON committees(normalized_name);
CREATE INDEX IF NOT EXISTS idx_committee_members_member_id ON committee_members(member_id);
CREATE INDEX IF NOT EXISTS idx_committee_members_congress ON committee_members(congress);
CREATE INDEX IF NOT EXISTS idx_committee_history_congress ON committee_history(congress);
...
-- Add trigger to committees table
...
```bash
psql -d project_tacitus_test -f migration_committees.sql
```

### Monitoring Setup

Set up monitoring for the committee scripts using a combination of logs and database tracking:

1. **Log Monitoring**:
   - Configure log rotation for the committee processing logs
   - Set up alerts for errors in the logs

2. **Database Monitoring**:
   - Create a dashboard for tracking committee data health
   - Monitor the `api_sync_status` table for failed syncs

3. **Script Health Checks**:
   Create a health check script that runs daily:
...
def check_committee_data_health():
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("""
                SELECT endpoint, status, last_sync_timestamp
                FROM api_sync_status
                WHERE endpoint LIKE 'committee%'
            ...
            """)
            missing_contact = cur.fetchone()[0]
            ...
            """)
            orphaned_subcommittees = cur.fetchone()[0]
            ...
            # Print health report
            logger.info("Committee Data Health Report")
            logger.info(f"Total committees: {total_committees}")
            logger.info(f"Total committee members: {total_members}")
            logger.info(f"Committees missing contact info: {missing_contact}")
            logger.info(f"Orphaned subcommittees: {orphaned_subcommittees}")
            logger.info(f"Recently updated committees: {recent_updates}")
            
            logger.info("Recent sync status:")
            for status in sync_statuses:
                logger.info(f"  {status[0]}: {status[1]} ({status[2]})")
            
            # Return overall health status
            has_errors = orphaned_subcommittees > 0 or any(s[1] == 'failed' for s in sync_statuses)
            return {
                'status': 'unhealthy' if has_errors else 'healthy',
                'total_committees': total_committees,
                'total_members': total_members,
                'missing_contact': missing_contact,
                'orphaned_subcommittees': orphaned_subcommittees,
                'recent_updates': recent_updates,
                'sync_statuses': [
                    {'endpoint': s[0], 'status': s[1], 'timestamp': s[2]} 
                    for s in sync_statuses
                ]
            }

if __name__ == "__main__":
    health = check_committee_data_health()
...
Set up a cron job to run committee data updates regularly:

```bash
# crontab -e

# Run committee fetch every day at 2:00 AM
0 2 * * * cd /path/to/project-tacitus && python3 congressgov/committee_fetch/committee_fetch_core.py >> logs/committee_fetch.log 2>&1

# Run committee detail processor every day at 3:00 AM
0 3 * * * cd /path/to/project-tacitus && python3 congressgov/committee_fetch/committee_detail_processor.py >> logs/committee_detail.log 2>&1

# Run committee member processor every day at 4:00 AM
0 4 * * * cd /path/to/project-tacitus && python3 congressgov/committee_fetch/committee_member_processor.py >> logs/committee_members.log 2>&1

# Run committee validation every day at 5:00 AM
0 5 * * * cd /path/to/project-tacitus && python3 congressgov/committee_fetch/committee_validation.py >> logs/committee_validation.log 2>&1

# Run health check every day at 6:00 AM
0 6 * * * cd /path/to/project-tacitus && python3 tools/committee_health_check.py >> logs/committee_health.log 2>&1
```

## Conclusion

This comprehensive committee processing system integrates seamlessly with the existing Project Tacitus architecture, following the same patterns used for bills and members. By leveraging the shared utilities and consistent design patterns, the committees module provides a robust and maintainable solution for tracking legislative committees, their membership, and their activities.

Key benefits of this implementation include:

1. **Consistent Architecture**: Follows the established patterns in the project for maximum maintainability
2. **Comprehensive Data**: Captures detailed committee information, including hierarchy, membership, and contact details
3. **Efficient Processing**: Uses incremental updates to minimize API calls and database load
4. **Data Validation**: Includes tools for monitoring data quality and filling gaps
5. **Performance Optimization**: Implements caching and database indexing for fast API responses
6. **Extensible Design**: Allows for future enhancements and additional data points

This implementation completes the legislative data triad of bills, members, and committees, providing a full picture of the legislative process for Project Tacitus users.# Committees API Documentation
